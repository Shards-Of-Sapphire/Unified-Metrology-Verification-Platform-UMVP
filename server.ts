...
const localStatePath = path.join(process.cwd(), 'data', 'local-state.json');
...
async function findCertificateByReference(idOrNumber: string): Promise<VerificationCertificate | null> {
  try {
  ...
  return certificates.find((c) => c.id === idOrNumber || c.certificateNumber === idOrNumber) || null;
}

async function buildCertificatePdfBuffer(cert: VerificationCertificate, publicBaseUrl: string): Promise<Buffer> {
...
async function storeCertificatePdfFallback(id: string, pdfData: string): Promise<{ stored: boolean; fallback: boolean }> {
...
const registeredUsers = new Map<string, LocalUserRecord>(
...
function persistLocalState() {
  try {
    ...
    fs.writeFileSync(
      localStatePath,
      JSON.stringify({ users: [...registeredUsers.values()], applications, certificates }, null, 2),
      'utf8'
    );
  } catch (error) {
...
app.post('/api/auth/signup', async (req, res) => {
  ...
  const maskedId = account.gstin
    ? (account.gstin.length > 6 ? `${account.gstin.slice(0, 4)}***${account.gstin.slice(-3)}` : account.gstin)
    : 'CITIZEN-USER';
...
app.post('/api/auth/account', authenticateToken, enforceRole(['CITIZEN']), async (req: AuthRequest, res) => {
  const { currentEmail, currentPassword, newEmail, newPassword } = req.body as {
    currentEmail?: string;
  ...
  return res.json({
    user: { id: updatedAccount.id, name: updatedAccount.name, email: updatedAccount.email, role: 'CITIZEN', maskedId: 'REGISTERED-USER', mfaVerified: true, loginTimestamp: new Date().toISOString(), token },
...
app.post('/api/auth/login', async (req, res) => {
  ...
  if (role === 'CITIZEN') {
    if (
      normalizedLoginEmail === 'rahul.sharma@apexlogistics.in' &&
      !registeredUsers.has(normalizedLoginEmail)
    ) {
      ...
      if (registered?.passwordHash) {
        ...
        const userGstin = (registered as any).gstin || '';
        const maskedId = userGstin
          ? (userGstin.length > 6 ? `${userGstin.slice(0, 4)}***${userGstin.slice(-3)}` : userGstin)
          : 'CITIZEN-USER';

        authenticatedUser = {
          id: (registered as any).id || (registered as any).uid,
          name: registered.name,
          email: registered.email,
          role: 'CITIZEN',
          phone: (registered as any).phone || '',
          businessName: (registered as any).businessName || '',
          gstin: userGstin,
          address: (registered as any).address || '',
          city: (registered as any).city || '',
          state: (registered as any).state || '',
          pincode: (registered as any).pincode || '',
          maskedId,
          mfaVerified: true,
          loginTimestamp: new Date().toISOString(),
        };
...
app.get('/api/applications', authenticateToken, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (dbApps && dbApps.length > 0) {
      // Map DB fields to MetrologyApplication type
...
app.post('/api/applications/:id/allocate', authenticateToken, enforceRole(['CONTROLLER_ADMIN']), async (req: AuthRequest, res: Response) => {
  ...
  applications[appIndex].allocatedLmoId = lmoId || 'lmo-malhotra-4091';
  applications[appIndex].allocatedLmoName = lmoName || 'Inspector Vikram Malhotra (Badge: DL-LMO-4091)';
  applications[appIndex].scheduledInspectionDate = scheduledDate || new Date().toISOString().split('T')[0];
  applications[appIndex].status = 'ALLOCATED';
  persistLocalState();

  // Sync to PostgreSQL
  try {
    await dbUpdateApplication(id, {
      allocatedLmoId: applications[appIndex].allocatedLmoId,
      allocatedLmoName: applications[appIndex].allocatedLmoName,
      scheduledInspectionDate: applications[appIndex].scheduledInspectionDate,
      status: 'ALLOCATED',
    });
  } catch (dbErr) {
  ...
  appendAuditBlock(
    ...
    'APPLICATION',
    id,
    `Allocated to ${applications[appIndex].allocatedLmoName} for verification date ${applications[appIndex].scheduledInspectionDate}`,
    req.ip
  );

  return res.json({ application: applications[appIndex], message: 'Inspection allocated to LMO.' });
});
...
app.post('/api/applications/:id/inspect', authenticateToken, enforceRole(['LMO']), async (req: AuthRequest, res: Response) => {
  ...
  applications[appIndex].fieldNotes = fieldNotes;
  applications[appIndex].inspectionGeotag = geotag || {
    latitude: 28.5283,
    longitude: 77.2711,
    timestamp: new Date().toISOString(),
    accuracyMeters: 3.5,
  };
  if (photoUrl) applications[appIndex].inspectionPhotoUrl = photoUrl;
  if (aiAnalysis) applications[appIndex].aiPhotoAnalysis = aiAnalysis;
  applications[appIndex].status = 'INSPECTION_COMPLETED';
  persistLocalState();
  ...
  try {
    await dbUpdateApplication(id, {
      fieldNotes,
      inspectionLatitude: applications[appIndex].inspectionGeotag?.latitude.toString(),
      inspectionLongitude: applications[appIndex].inspectionGeotag?.longitude.toString(),
      inspectionGeotagTimestamp: applications[appIndex].inspectionGeotag?.timestamp,
  ...
  appendAuditBlock(
    ...
    'APPLICATION',
    id,
    `Field inspection recorded with geotag (${applications[appIndex].inspectionGeotag?.latitude}, ${applications[appIndex].inspectionGeotag?.longitude}). AI Seal Status: ${aiAnalysis?.sealStatus || 'PASSED'}.`,
    req.ip
  );

  return res.json({ application: applications[appIndex], message: 'Inspection record saved.' });
});
...
app.put('/api/certificates/:id/pdf', authenticateToken, enforceRole(['LMO', 'CITIZEN']), async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  ...
  const cert = certificates.find((item) => item.id === id) || await dbGetCertificateById(id);
...
app.get('/certificates/:id/download.pdf', async (req: Request, res: Response) => {
...
app.get('/verify/:id', async (req: Request, res: Response) => {
  const cert = await findCertificateByReference(req.params.id);
  if (!cert) {
...
app.get('/api/certificates/:id/pdf', authenticateToken, enforceRole(['LMO', 'CITIZEN']), async (req: AuthRequest, res: Response) => {
  const cert = await findCertificateByReference(req.params.id);
  if (!cert) return res.status(404).json({ error: 'Certificate not found.' });
...
// Server info endpoint – returns the LAN IP so the frontend can build scannable QR URLs
