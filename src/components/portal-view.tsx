  ...
  return <main className="portal-shell">
    ...
    <section className="content-area">
      ...
      <div className="page-body workflow-body"><div className="heading-row"><div><p className="eyebrow">ANDHRA PRADESH / OPERATIONS</p><h1>{view}</h1><p className="lede">{view === "Applications" ? "Track every verification request from submission to decision." : view === "Inspections" ? "Plan field work, record evidence, and complete reviews." : view === "Certificates" ? "Issue, search, and manage QR-enabled verification certificates." : "Create and download operational reports for your department."}</p></div><div className="sync-status"><span className="status-dot" />Synced just now</div></div>
        ...
        {view === "Certificates" && <section className="panel workflow-panel"><div className="lookup-banner"><div><strong>Public certificate verification</strong><span>Search by certificate ID or scan a QR code.</span></div><button className="export-button" onClick={() => action("Verification lookup opened.")}>Verify certificate</button></div><div className="table-head"><span>CERTIFICATE</span><span>HOLDER</span><span>VALIDITY</span><span>STATUS</span><span> </span></div>{dataLoading ? <div className="empty-state">Loading certificates...</div> : certificates.filter((item) => item.join(" ").toLowerCase().includes(query.toLowerCase())).map((item) => <div className="table-row" key={item[0]}><strong>{item[0]}</strong><span>{item[1]}<small>{item[2]}</small></span><span>{item[3]}<small>until {item[4]}</small></span><span className={`status-pill ${item[6]}`}>{item[5]}</span><button className="row-arrow" onClick={() => action(`${item[0]} ready to view or download.`)} aria-label={`Open ${item[0]}`}>&gt;</button></div>)}</section>}
