export type UserRole = 'CITIZEN' | 'LMO' | 'CONTROLLER_ADMIN' | 'GATC';
...
export interface GatcTestItem {
  ...
  model: string;
  technician: string;
  status: 'SCHEDULED' | 'IN_PROGRESS' | 'PASSED' | 'FAILED_RETEST';
}

export interface GatcActionItem {
  id: string;
  title: string;
  category: 'CALIBRATION_APPROVAL' | 'RETEST_AUTHORIZATION' | 'URGENT_INSPECTION';
  dueTime: string;
  priority: 'HIGH' | 'MEDIUM';
}

export interface GatcEquipmentAlert {
  id: string;
  equipmentName: string;
  equipmentCode: string;
  issue: string;
  status: 'WARNING' | 'MAINTENANCE_REQUIRED' | 'OPERATIONAL';
  dueDate?: string;
}

export interface GatcAuthorityNotice {
  id: string;
  title: string;
  issuer: string;
  date: string;
